...
class TextDetector:
    ...
    def __remove_nonchars(self):
        ...
        if self.do_visualize:
            key1, key2, key3 = "Threshold", 'Components', "Removed"
            fig, ax = self.__make_subplot_figure([key1, key2, key3], title = "2: Remove non-chars")
        ...
        if self.do_visualize: self.__make_subplot(outputMask, ax, key2, title = "Rejected Components")

        # Subtract mask from image
        self.image = cv2.subtract(self.image, outputMask)
        if self.do_visualize: self.__make_subplot(self.image, ax, key3, title = "Non-chars filtered")

    def __secondary_processing(self):
        if self.do_visualize:
            key1, key2, key3 = "RemoveVerticalLines", 'RemoveHorizontalLines', "Removed"
